...
export class SendRemittanceUseCase implements UseCase<SendRemittanceInput, SendRemittanceOutput> {
  ...
  private async doExecute(input: SendRemittanceInput): Promise<Remittance> {
    ...
    const sourceWallet = await this.walletRepository.findByAccountIdAndCurrency(
      senderAccountId,
      sourceCurrency
    );
    if (!sourceWallet) {
    ...
    const destinationWallet = await this.walletRepository.findByAccountIdAndCurrency(
      recipientAccountId,
      destinationCurrency
    );
    ...
    if (!compliance.approved) {
      throw new ComplianceRejectedError(
        compliance.reason ?? 'sender not approved to send this amount'
      );
    }
    ...
    const treasuryDestinationWallet = isSameCurrency
      ? treasurySourceWallet
      : await this.getTreasuryWallet(destinationCurrency);

    const convertedAmount = isSameCurrency
      ? amount
      : (await this.exchangeRateProvider.getRate(sourceCurrency, destinationCurrency)).convert(
          amount
        );
    ...
    if (isSameCurrency) {
      ...
      legs.push(
        {
          ...
          money: amount,
          description: 'remittance principal debit',
        },
        {
          walletId: destinationWallet.getId(),
          direction: EntryDirection.credit(),
          money: amount,
          description: 'remittance principal credit',
        },
        {
          walletId: sourceWallet.getId(),
          direction: EntryDirection.debit(),
          money: fee,
          description: 'remittance fee debit',
        },
        {
          walletId: treasurySourceWallet.getId(),
          direction: EntryDirection.credit(),
          money: fee,
          description: 'remittance fee revenue',
        }
      );
      ...
      legs.push(
        ...
        },
        {
          ...
          direction: EntryDirection.credit(),
          money: amount,
          description: 'FX settlement (source leg)',
        },
        {
          walletId: sourceWallet.getId(),
          direction: EntryDirection.debit(),
        ...
        {
          walletId: treasuryDestinationWallet.getId(),
          direction: EntryDirection.debit(),
          money: convertedAmount,
          description: 'FX settlement (destination leg)',
        },
        {
          walletId: destinationWallet.getId(),
          direction: EntryDirection.credit(),
  ...
  private async getTreasuryWallet(currency: Currency): Promise<Wallet> {
    const wallet = await this.walletRepository.findByAccountIdAndCurrency(
      TREASURY_ACCOUNT_ID,
      currency
    );
    if (!wallet) {
