...
export async function seedTreasuryWallets(walletRepository: WalletRepository, clock: Clock): Promise<void> {
  for (const code of Currency.supportedCodes()) {
    const currency = Currency.from(code)
    const existing = await walletRepository.findByAccountIdAndCurrency(TREASURY_ACCOUNT_ID, currency)
