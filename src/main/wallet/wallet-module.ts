...
): UseCase<OpenWalletInput & { idempotencyKey: string }, OpenWalletOutput> {
  const openWalletUseCase = new OpenWalletUseCase(deps.walletRepository, deps.clock);

  const idempotentOpenWallet = new IdempotentDecorator(
    openWalletUseCase,
